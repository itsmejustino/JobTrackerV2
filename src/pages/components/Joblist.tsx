...
const JobList: FC = () => {
  ...
  const editJob = async (
    ...
    editJobMutation(
      { id, userId, jobName, company, platform, appliedon, interview, followup }
    ).then(() => {
      queryUserJobList.refetch();
    });
  };
      ...
      return (
        ...
        >
          <div className="m-4 flex flex-col p-2">
            ...
            <button
              onClick={() => {
                editJob(x.id, userId, formData.jobName, formData.company, formData.platform, formData.appliedon, formData.interview, formData.followup);
              }}
              type="button"
              className="m-4 flex flex-row justify-center gap-2 rounded-md bg-blue-400 p-2 text-sm shadow-md transition hover:bg-blue-500"
              key={x.id}
